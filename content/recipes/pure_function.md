...
嗯... 这里我们可以肯定第二个测试 `testIncreaseB` 会成功。`0 + 1` 肯定等于 `1`。那第一个测试呢？这可不好说了，我们并不知道  `testIncreaseA` 是在什么环境下被调用的，不知道在这个环境下初始 `state` 是多少。如果他是 `0` 那测试就会成功的，如果他不是 `0` 那测试就会失败的。因此在不知道所处环境时，我们无法判断测试是否会成功。
...
而我们将这些作用称为函数的 [附加作用]（副作用）。

而 [纯函数] 的定义就是： **没有 [附加作用] 的函数，并且在参数相同时，返回值也一定相同。**

因此在已知函数执行逻辑时，函数的返回值是**可以被预测**出来的。一些现代话的库都利用了这个特性来做状态管理，如：[RxFeedback]， [Redux]，[ReactorKit] 等等。
...
这个例子似乎过于简单，以至于我们看不出他有什么特别的。好吧，我承认这个例子的主要目的是告诉大家，用 [纯函数] 做状态管理的**基本单元是什么**。
...
func testReduce() {
    ...
    XCTAssertEqual(state1, 1) // 😎

    let state2 = reduce(10, event: .decrease)
    XCTAssertEqual(state2, 9) // 😎
}
```
...
当程序处于某个特定状态时，发生了某个特定事件，会产生某个**唯一的结果**。而这个结果与发生的环境无关，不论是发生在应用程序运行环境，还是在测试环境。这个结果只和初始状态以及发生的事件有关。因此，程序的行为是可以被预测的，而且程序运行时的状态更新，可以在测试环境中被模拟出来。
...
func testOnUpateUsername() {
    ...
    XCTAssertEqual(newState, expect) // 😎
}
```
...
现在，我们知道如何用 [纯函数] 做状态管理了。不过当前的代码状态，离投入生产环境，还有好几个过度状态。这些过度状态主要都是围绕如何引入 [附加作用]，而做了一些应用架构。而且在这个问题上，不同地架构也提出了不同的解决方案，如：[RxFeedback] 用 `feedbackLoop` 引入 [附加作用]，[Redux] 用 `middleware` 引入 [附加作用] 等等。这里就不一一介绍了，这些库的官方网站都会有相关说明。

最后，我们还是将代码演化到下一个状态，这里我选择使用 [Redux] 流派。因为个人的觉得他的知识依赖要少一些，可以让更多读者从中获益。
...
class Store {
    ...
    func dispatch(event: State.Event) {
        ...
        publish(oldState: _oldState, newState: self.state)
    }
    
    func subscribe(observer: @escaping Observer) -> UUID {
        let subscriptionID = UUID() //  UUID 是唯一标识符
        _observers[subscriptionID] = observer
        observer(nil, self.state)
    ...
    private func publish(oldState: State?, newState: State) {
...
如果我们不知道这个类的作用，以及其公有方法的作用。可以先往下面看，后面会解释如何使用 `Store`:
...
func useStore() {
    ...
    XCTAssertEqual(newStates, expectNewStates) // 😎
    XCTAssertEqual(oldStates, expectOldStates) // 😎
}
```
...
这就是如何在测试环境里面使用 `Store`，那么在 `App` 里面如何使用 `Store` 呢。 一个相对简单的方法，就是将 `Store` 注入到对应的组件里面，这里以 `ViewController` 为例, `ViewController` 可以使用 `store.subscribe` 方法订阅程序的状态，当状态更新时，刷新 UI。然后，当用户触发某个事件时，调用 `store.dispatch` 方法将事件发出去，如：当用户点击登录按钮时，就调用 `store.dispatch(event: .onTriggerLogin)`。最后，需要在 `ViewController` 的 `deinit` 方法里面注销订阅 `store.unsubscribe(subsriptionID)`。
